...
export class Filters {
    ...
    public generateFilter(data: IProduct[], filterType: FilterType): void {
        ...
        const setFilter: Set<string> = this.generateFilterItems(
            data,
            filterType
        );
        setFilter.forEach((item) =>
            filter.appendChild(this.createCheckbox(item, filterType))
        );
    }

    private generateFilterItems(
        data: IProduct[],
    ...
    private createCheckbox(value: string, filterType: FilterType): HTMLElement {
        ...
        checkbox.addEventListener('click', () =>
            this.viewFiltersResult(checkbox.id, filterType)
        );
    ...
    private viewFiltersResult(
        ...
        productsData.forEach((product) => {
            ...
            for (const key of this.selectedFiltres.keys()) {
                const filterValuesForType: Set<string> =
                    this.selectedFiltres.get(key)!;
                const productPropertyValue: string = product[key].toLowerCase();
                if (
                    filterValuesForType.size !== 0 &&
                    !filterValuesForType.has(productPropertyValue)
                ) {
                    addToResult = false;
                    break;
                }
